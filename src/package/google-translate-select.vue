<template>
  <div
    ...
    :class="getClass"
    @mouseover="handleDropdownShowByHover"
...
export default {
          ...
          records.forEach((record) => {
            if (record.addedNodes[0] && record.addedNodes[0].value) {
              if (this.selectedLanguageCode === record.addedNodes[0].value) {
                this.triggerTranslate(record.addedNodes[0].value);
              }
            }
          });
        },
  ...
  mounted() {
    this.googleTranslateSelectEl = this.$refs.googleTranslateSelectEl;
